...
import style from "@/app/(admin)/dashboard/product/style.module.css";
import { useState } from "react";
import { BASE_URL, ACCESS_TOKEN } from "@/app/library/constants";
...
export default function Product({ product, method }:Props) {
  ...
  const handleSubmitProudct = async (value: ProductPostType & { id: string }) => {
    const res = await fetch(`${BASE_URL}/api/products/${value.id}`, {
      method: `${method}`,
      headers: {
    ...
    <main className={`${style.container}`}>
      <Formik
        ...
        onSubmit={async (values) => {
          ...
          // create product post
          const productPost: ProductPostType & { id: string } = {
...
const CustomInput = ({ field, setFieldValue }: any) => {
  ...
  return (
    <div>
      ...
      <input
        id="fileInput"
        onChange={(e) => handleUploadeFile(e)}
        type="file"
      />
